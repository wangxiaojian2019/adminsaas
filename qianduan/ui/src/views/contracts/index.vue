  ...
  <div class="contracts-container">
    <el-card shadow="never" class="main-card">
      ...
      <el-table :data="processedTableData" v-loading="loading" border stripe style="width: 100%">
        ...
        <el-table-column label="交租标准" width="110" align="center">
          <template #default="{ row }">
            <el-tag effect="plain" type="warning">付 {{ row.payment_cycle }} 个月</el-tag>
          </template>
        </el-table-column>
    ...
    <el-drawer v-model="docsDrawerVisible" :title="`电子合同档案：${currentContract.contract_no}`" size="500px">
      <div class="docs-container" v-loading="docsLoading">
        ...
        <ul class="doc-list">
          <li class="doc-item">
            <div class="doc-info"><el-icon class="doc-icon"><Document /></el-icon><div class="doc-name-time"><span class="doc-name">电子制式合同</span></div></div>
            <div class="doc-ctrl"><el-button v-if="currentDocs.elec_contract_url" type="primary" link @click="previewDoc(currentDocs.elec_contract_url)">预览/下载</el-button><span v-else style="font-size: 12px; color: #909399;">未生成</span></div>
          </li>
          <li class="doc-item">
            <div class="doc-info"><el-icon class="doc-icon"><Picture /></el-icon><div class="doc-name-time"><span class="doc-name">线下扫描归档件</span></div></div>
            <div class="doc-ctrl"><el-button v-if="currentDocs.paper_contract_url" type="primary" link @click="previewDoc(currentDocs.paper_contract_url)">预览/下载</el-button><span v-else style="font-size: 12px; color: #909399;">未上传</span></div>
          </li>
        </ul>
      </div>
    </el-drawer>
...
// 数据集注入新增 payment_cycle (默认 3 个月即季付)
...
const currentDocs = ref({ elec_contract_url: null, paper_contract_url: null })
...
const checkoutForm = reactive({ refund_deposit: 0, deduct_rent: 0, deduct_damage: 0 })

// ---- 核心新增引擎：业财智能计算 ---- //

// 智能填充押金（默认 1 个月租金，用户可自行改写）
...
const syncCustomCycle = (val) => {
  // 当自定义框数值变化时，底层数据保持 0，实际提取通过 actualPaymentCycle
}
...
const fetchDocs = async (id) => {
  ...
  if (res.code === 200) currentDocs.value = res.data || { elec_contract_url: null, paper_contract_url: null }
